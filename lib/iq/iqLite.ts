...
export type IQInput = {
  name: string;
  target: string; // e.g. "8-12" or "10"
  events: { sessionId: string | null; kg: number; reps: number; rir: number | null; ts: number; type: string }[];
...
export function iqLite(input: IQInput): { stage: IQStage; load: number | null; why: string; exposures: number; confidence: number } {
  ...
  const comparable = events
    .filter((e) => e.sessionId && finalizedIds.has(String(e.sessionId)) && e.type !== "W" && e.kg > 0 && e.reps > 0)
    .filter((e) => String((e as unknown as { name?: string }).name || name) === name || true) // if name not in event, assume match for test
    .sort((a, b) => (b.ts || 0) - (a.ts || 0));

  // For test simplicity, if events don't have name, treat all as matching
  const filtered = comparable.length ? comparable : events.filter((e) => e.kg > 0 && e.reps > 0).sort((a, b) => (b.ts || 0) - (a.ts || 0));
  ...
  if (exposures < 3 || !prev) {
    return {
      stage: "CALIBRATE",
      load: prev ? Number(prev.kg) : null,
      why: `Baseline ${exposures}/3 sesi sebanding.`,
      exposures,
